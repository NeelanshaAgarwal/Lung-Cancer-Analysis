...
def predict():
        if request.method == 'POST':
            ...
            output = prediction
            if output == 0:
                return render_template('index.html', pred='Person Has Lung Cancer '.format(output))
            else:
                return render_template('index.html', pred='Person Does Not Got Lung Cancer'.format(output))
        else:
            return render_template('index.html')
